describe <- function(# Arguments de base
  ...
  tab.res.moy <- function(x, y, conf.level, decimal, mean.test, include.p, conf.method.num = conf.method.num ){
  ...
  tab.res.fac <- function(x, y, conf.level, conf.method.cat, decimal, simulate.p.value, chi.correct, prop.type, prop.test, include.p,...) {
  ...
  tri_croise_map_fun <- function(vars, factor, decimal, conf.level, num.type, prop.type, prop.test, chi.correct, simulate.p.value,
    ...
    if (is.numeric(x)) {
      switch(num.type,  # Appelle des fonctions correspondante selon "num.type"
             mean = {return(tab.res.moy(x = x, y = y, conf.level = conf.level, decimal = decimal, mean.test = mean.test, include.p = include.p, conf.method.num = conf.method.num))},
             ...
             both = {return(bind_rows( # concatenation des deux precedentes
               tab.res.moy(x = x, y = y, conf.level = conf.level, decimal = decimal, mean.test = mean.test, include.p = include.p, conf.method.num = conf.method.num),
               tab.res.med(x = x, y = y, conf.level = conf.level, decimal = decimal, include.p = include.p, conf.method.num = conf.method.num)
             ))})
    }
    if (is.factor(x)) {
      return(tab.res.fac(x = x, y = y, conf.level = conf.level, conf.method.cat = conf.method.cat, decimal = decimal, simulate.p.value = simulate.p.value,
  ...
  if (is.defined(factor)) { # Si une variable stratifiante est renseignee
    ...
    if (length(vars) == 1) { # Si une seule variable explicative, on lance la fonction une seule fois
      res <- list(tri_croise_map_fun(factor = data[, factor], vars = data[, vars], decimal = decimal, prop.type = prop.type, prop.test = prop.test,
      ...
      res <- purrr::map(data[, vars], tri_croise_map_fun, factor = data[, factor], decimal = decimal, prop.type = prop.type, prop.test = prop.test,
    ...
    res <- purrr::map(data[, vars], tri_croise_map_fun, factor = data[, factor], decimal = decimal, prop.type = prop.type,
