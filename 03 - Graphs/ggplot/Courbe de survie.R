...
data$etime <- pmin(data$delai_dc, data$delai_hosp)
data$event <- factor(ifelse(data$delai_dc < data$delai_hosp, "Décès",
                            ifelse(data$delai_hosp == 365, "Censure", "Hospitalisation")),
                     levels = c("Censure", "Hospitalisation", "Décès"))
mfit_expo <- survfit(Surv(etime, event) ~ EXPO, data = data)

ymax <- 70
p <- autoplot(mfit_expo, surv.linetype = 'blank', surv.colour = NA) +
  geom_step(aes(linetype = strata, color = event), size = 0.8) +
  scale_linetype_manual(name = "Groupe",
                        values = c('dotted', 'solid'),
                        labels = c('Non-exposé', 'Exposé')) +
  scale_color_manual(name = "Etat",
                     values = c("white", "#1C4073", "#B28B84"),
                     labels = c(' ', "Hospitalisation", 'Décès')) +
  labs(x = "Période de suivi (jours)", y = "Proportion de la population", title = "") +
